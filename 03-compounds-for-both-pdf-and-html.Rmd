...
With the new data, we can find that the selected models still all good at capturing UTD. And comparing the UTD values of the two sets of models in Table \@ref(tab:tab1) and Table \@ref(tab:tab4), it is easy to see that the Copula model using Anomalies has improved.
...
Then looking again to Bundesland, in Table \@ref(tab:tab5) both tau and UTD have improved with respect to the previous data in Table \@ref(tab:tab2), and even in Schleswig-Holstein now shows a correlation between the two variables.
...
In Table \@ref(tab:tab6), the overall lowering of UTD has also become smaller, but Rheinland-Pfalz and Saarland, while still maintaining kendalls tau, have lost UTD, but this is much more acceptable compared to the previous overall sharp fall.
