...
```{r fig1_gamze, cache=FALSE, out.width="500", fig.align="center", echo=FALSE, eval = TRUE, fig.cap="Schematic depicting the design low-flow quantile $z_{p_{0}}$ with (a) constant exceedance probability $p_{0}$, and (b) time-varying exceedance probabilities $p_{t}, t=1,2, \\ldots, \\infty$."}
...
& =p_{x} \prod_{t=1}^{x-1}\left(1-p_{t}\right), \quad x=1,2, \ldots, \infty \tag{8}
\end{align*}
$$
...
```{r fig2_gamze, cache=FALSE, out.width="500", fig.align="center", echo=FALSE, eval = TRUE, fig.cap="Nonstationary return period T and hydrological risk R of the Wei River"}
...
&= \frac{\lambda}{P\left\{C\left[F_{1}\left(y_{1} \mid \boldsymbol{\theta}_{1}\right), F_{2}\left(y_{2} \mid \boldsymbol{\theta}_{2}\right) \mid \theta_{c}\right] < p_{KEN}\right\}} \tag{23} \\
...
$$
\begin{gather*}
T_{A N D}^{t}=\frac{1}{P\left(Y_{1} \leq y_{1} \wedge Y_{2} \leq y_{2}\right)}  \tag{24}\\
...
=\frac{1}{F_{1}\left(y_{1} \mid \boldsymbol{\theta}_{1}^{t}\right)+F_{2}\left(y_{2} \mid \boldsymbol{\theta}_{2}^{t}\right)-C\left[F_{1}\left(y_{1} \mid \boldsymbol{\theta}_{1}^{t}\right), F_{2}\left(y_{2} \mid \boldsymbol{\theta}_{2}^{t}\right) \mid \theta_{c}^{t}\right]} \tag{25}
\end{gather*}
$$
...
$$
\begin{equation*}
=\frac{1}{P\left\{C\left[F_{1}\left(y_{1} \mid \boldsymbol{\theta}_{1}^{t}\right), F_{2}\left(y_{2} \mid \boldsymbol{\theta}_{2}^{t}\right) \mid \theta_{c}^{t}\right]<p_{K E N}\right\}} \tag{26}
\end{equation*}
$$
...
```{r fig3_gamze, cache=FALSE, out.width="500", fig.align="center", echo=FALSE, eval = TRUE, fig.cap="The isolines of design low-flow events with JRP=50 years for three different time periods of 1954-1967, 1968-1991 and 1992-2011"}
...
$$
\begin{align*}
H_{Y_{1}, Y_{2}}\left(y_{1}^{t}, y_{2}^{t}\right) & =C\left[F_{1}\left(y_{1}^{t} \mid \boldsymbol{\theta}_{1}^{t}\right), F_{2}\left(y_{2}^{t} \mid \boldsymbol{\theta}_{2}^{t}\right) \mid \theta_{c}^{t}\right]  \tag{27}\\
...
$$
\begin{equation*}
g_{c}\left(\theta_{c}^{t}\right) = \beta_{0} + \sum_{i=1}^{m} \beta_{i} x_{i}^{t} \tag{28}
\end{equation*}
$$
...
```{r fig4_gamze, cache=FALSE, out.width="500", fig.align="center", echo=FALSE, eval = TRUE, fig.cap="Schematic illustration of SDSM"}
...
```{r fig5_gamze, cache=FALSE, out.width="500", fig.align="center", echo=FALSE, eval = TRUE, fig.cap="SDSM Climate Scenario Generation"}
...
$$
\begin{align*}
y_{i}^{U C}=\gamma_{0}+\sum_{j=1}^{l} \gamma_{j} \hat{u}_{i}^{j}+\varepsilon \tag{29}
\end{align*}
$$
