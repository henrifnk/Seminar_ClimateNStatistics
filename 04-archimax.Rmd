...
Also for asymmetry measures defined by equation \@ref(eq:asymmeassupgen), an upper bound exists for every copula \(C\) (see @durante2010a). Specifically, it holds that
...
Moreover, Archimax copulas can also be symmetric, in which case \(\nu_\infty(C_{\phi, A})\) equals 0 (see axiom A2 in section \@ref(backgrmeas)), which is the smallest value \(\nu_\infty(C)\) can assume for any copula \(C\). Thus, table \@ref(tab:archiBoundsClaudia) shows that the range of asymmetry of Archimax copulas is wider than that of extreme value copulas, which corresponds to the conclusion drawn by @durante2010b. Consequently, Archimax copulas in their general form are more flexible in modelling asymmetric dependence structures than one of their special cases, the extreme value copulas. This might be required in some applications.
