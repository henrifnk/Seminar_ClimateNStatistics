...
\citet{Lopez2013} proposed the reservoir index (RI) as an indicator of the impact of reservoirs on flow regimes. It is defined as follows:

\begin{equation}
RI = \sum_{i=1}^{N}\left(\frac{A_{i}}{A_{T}}\right) \cdot \left(\frac{V_{i}}{C_{T}}\right)
\label{eq:RI}
\end{equation}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.0650}}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.4150}}@{}}
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\citet{Du2015} & Observed Hydrological Data & Mean daily streamflow data & 1954-2009 & Huaxian and Xianyang gauging stations \\
...
\citet{Wang2022} & Observed Hydrological Data & Mean daily streamflow data & 1955-2016 & Multiple gauging stations in the Yellow River Basin \\
...
\citet{Jiang2015} & Observed Hydrological Data & Mean daily streamflow data & 1961-2010 & Ankang (Qa) and Huangzhuang (Qh) gauges \\
...
\begin{equation}
U_{t, N}=\sum_{j=1}^{t} \sum_{i=1}^{N} \operatorname{sgn}\left(x_{j}-x_{i}\right) \quad (t=1, \quad \cdots, \quad N)
\label{eq:Pettittind}
\end{equation}
...
\caption{Schematic depicting the design low-flow quantile $z_{p_{0}}$ with (a) constant exceedance probability $p_{0}$, and (b) time-varying exceedance probabilities $p_{t}, t=1,2, \ldots, \infty$.}\label{fig:fig1Gamze}
\end{figure}
...
\begin{equation}
f(x)=P(X=x)=\left(1-p_{0}\right)^{x-1} p_{0}, \quad x=1,2, \ldots, \infty
\label{eq:ewtstatpdf}
\end{equation}

Given that Equation \eqref{eq:ewtstatpdf} is based on the assumptions of independence and stationarity, the expected value of \(X\)---representing the return period (expected waiting time interpretation) of the low-flow exceeding the design quantile \(z_{p_{0}}\) under stationary conditions, is:

\begin{equation}
T=E(X)=\sum_{x=1}^{\infty} x f(x)=1 / p_{0}
\label{eq:ewtstat}
\end{equation}

\textbf{Under nonstationary conditions}, the exceedance probability associated with \(z_{p_{0}}\) is no longer constant (Figure \ref{fig:fig1Gamze}b). Consequently, the geometric probability law, which takes into account time-varying exceedance probabilities \(p_{t}\), is expressed as (\citet{Cooley2013}, \citet{Salas2013}, \citet{Salas2014}):
...
\begin{equation}
 T=E(X)=\sum_{x=1}^{\infty} x f(x)=\sum_{x=1}^{\infty} x p_{x} \prod_{t=1}^{x-1}\left(1-p_{t}\right)
\label{eq:ewtnonstat}
\end{equation}
...
\begin{equation}
f(m)=P(M=m)=\binom{T}{m} p_{0}^{m}\left(1-p_{0}\right)^{T-m}
\label{eq:enestatpdf}
\end{equation}
...
The ENE-return period \(T\) for a low-flow event exceeding the design quantile \(z_{p_{0}}\) under nonstationary conditions can be derived by setting Equation \eqref{eq:enenonstat} equal to 1 and solving:

\begin{equation}
\sum_{t=1}^{T} p_{t}=1
\label{eq:enenonstat1}
\end{equation}
...
\begin{equation}
R=1-\left[\left(1-p_{1}\right)\left(1-p_{2}\right) \ldots\left(1-p_{n}\right)\right]=1-\prod_{t=1}^{n}\left(1-p_{t}\right)
\label{eq:risknonstat}
\end{equation}

\textbf{Return Period and Risk Results from \citet{Du2015}}

\textbf{Nonstationary Return Period Using Meteorological Covariates}

Figure \ref{fig:fig2Gamze} presents the return period and risk results using meteorological covariates (explained in the model selection part) for Huaxian and Xianyang stations.
...
\caption{Nonstationary return period T and hydrological risk R of the Wei River}\label{fig:fig2Gamze}
\end{figure}
...
The standard method for calculating nonstationary return periods under the EWT and ENE interpretations, as well as the risk of a design quantile \(z_{p_{0}}\) corresponding to an initial return period \(T_{0}\), is outlined by Equations \eqref{eq:ewtnonstat}, \eqref{eq:enenonstat1}, \eqref{eq:risknonstat}, respectively. A crucial aspect of this procedure is the derivation of time-varying exceedance probabilities \(p_{t}\) (Equation \eqref{eq:exprobnonstat}) for future years. This process involves determining the relationships between the statistical parameters of the low-flow distribution and the explanatory variables, which is typically referred to as nonstationary frequency analysis.
...
\begin{equation}
g_{k}(\theta_{k}) = \eta_{k} = X_{k} \beta_{k} + \sum_{j=1}^{J_{k}} Z_{jk} \gamma_{jk}
\label{eq:gamlss}
\end{equation}
...
\begin{equation}
g_{k}(\theta_{k}) = \eta_{k} = X_{k} \beta_{k} + \sum_{j=1}^{J_{k}} h_{jk}(x_{jk})
\label{eq:gamlsssemiparam}
\end{equation}
...
Various probability distributions have been suggested for modeling low-flow events (\citet{Matalas1963}; \citet{Eratakulan1970}; \citet{Smakhtin2001}; \citet{Hewa2007}; \citet{Liu2015}). \citet{Matalas1963} investigated four distributions in modeling low-flow data from 34 streams and found that the Gumbel and Pearson-III distributions fit the data well and were more representative than the Lognormal and Pearson-V distributions. \citet{Eratakulan1970} discovered that the Gamma and Weibull distributions were the most suitable for modeling the low-flow series from 37 stations in the Missouri River basin. \citet{Hewa2007} introduced the GEV distribution into the frequency analysis of low-flow data from 97 catchments in Victoria, Australia. \citet{Liu2015} tested six distributions for modeling the annual low flows at the Yichang station, China, under nonstationary conditions and found that the GEV distribution provided the best fit.

Based on these studies, five two-parameter distributions (Gamma (GA), Weibull (WEI), Gumbel (GU), Logistic (LO), and Lognormal (LOGNO)) and two three-parameter distributions (Pearson-III (P-III) and GEV) that are widely used in modeling low-flow data are listed in \hyperref[table7.1Gamze]{Table 7.1}. Considering that the shape parameter \(\kappa\) of the P-III and GEV distributions is quite sensitive and difficult to estimate, it is mostly assumed to be constant (\citet{Coles2001}; \citet{Katz2002}; \citet{Gilroy2012}; \citet{Du2015}). Nonstationarities in both the location \(\mu\) and scale \(\sigma\) parameters are examined through monotonic link functions \(\mathrm{g}(\cdot)\).

\phantomsection\label{table7.1Gamze}
\begin{longtable}[]{@{}
...
\textbf{Table 7.1: Summary of the distributions used to model the low-flow series (\citet{Du2015})}
...
\begin{equation}
\mathrm{BIC}=-2 \ln (\mathrm{ML})+k \ln(n)
\label{eq:BIC}
\end{equation}
...
\[
b_{i}=\left\{\begin{array}{lc}1-b_{\tau} & i=1 \\ (i-0.3175) /(\tau+0.365) & i=2,3, \ldots, \tau-1 \\ 0.5^{(1 / \tau)} & i=\tau\end{array}\right.
\]
...
\begin{equation}
D_{K S}=\max _{1 \leq i \leq \tau}\left|\hat{G}_{i}-G_{(i)}\right|
\label{eq:KS}
\end{equation}
...
\item
  Deriving time-varying exceedance probabilities \(p_{t}\) corresponding to \(z_{p_{0}}\) for future years \(t=1,2, \ldots, \infty\) (when using time as a covariate) or \(t=1,2, \ldots, t_{\max }\) (when using meteorological covariates) from Equation \eqref{eq:exprobnonstat}) where \({\theta}_{t}\) is calculated by extending the optimal nonstationary model from step (i) into the future under the respective case of covariates.
\end{enumerate}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.0271}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.0710}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.0564}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.1837}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.6555}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Study
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
where \(\lambda\) is the average interarrival time between low-flow event occurrences. It is assumed here that the annual minimum low-flow series is investigated, so \(\lambda\) is set to 1 (i.e., \(\lambda = 1\)). \(K_{C}(\cdot)\) is the Kendall distribution function (\citet{Genest1993}, \citet{Barbe1996}, \citet{Salvadori2007}), which provides a univariate representation of multivariate information, and \(p_{KEN}\) is a critical probability level corresponding to the value of \(K_{C}(p_{KEN})\) (\citet{Salvadori2011}, \citet{Salvadori2013}).
...
&= \frac{1}{P\left\{C\left[F_{1}\left(y_{1} \mid \boldsymbol{\theta}_{1}^{t}\right), F_{2}\left(y_{2} \mid \boldsymbol{\theta}_{2}^{t}\right) \mid \theta_{c}^{t}\right] < p_{K E N}\right\}} \\
&= \frac{1}{K_{C}^{t}\left(p_{K E N}\right)}
\end{split}
\label{eq:jpr}
\end{equation}

\textbf{Joint Return Period in \citet{Jiang2015}}

\citet{Jiang2015} analyzed the joint return period of low-flow events at the Ankang (Qa) and Huangzhuang (Qh) gauges on the Hanjiang River using a time-varying copula model. The study used three methods to calculate the JRP: AND, OR, and Kendall (KEN) methods.
...
\caption{The isolines of design low-flow events with JRP=50 years for three different time periods of 1954-1967, 1968-1991 and 1992-2011}\label{fig:fig3Gamze}
\end{figure}
...
According to \eqref{eq:copula}, three scenarios for the time-varying copula can be derived:
...
The implementation of the time-varying copula model in \eqref{eq:copula} involves two major steps: first, determining the time variation in the marginal distributions, and second, modeling the evolution of the copula parameter. The first step is done under the GAMLSS framework we have discussed before. As the second step the copula parameter \(\theta_{c}\) can be expressed as a linear function of the time-varying explanatory variables \(x_{i}^{t} (i=1,2, \ldots, m)\) through a suitable link function \(g_{c}(\cdot)\), as follows:

\begin{equation}
g_{c}\left(\theta_{c}^{t}\right) = \beta_{0} + \sum_{i=1}^{m} \beta_{i} x_{i}^{t}
\label{eq:copulalinkfunc}
\end{equation}
...
\caption{SDSM Climate Scenario Generation}\label{fig:fig5Gamze}
\end{figure}
...
\begin{equation}
y_{i}^{U C}=\gamma_{0}+\sum_{j=1}^{l} \gamma_{j} \hat{u}_{i}^{j}+\varepsilon
\label{eq:sdsmuc}
\end{equation}
...
\begin{equation}
\omega_{i}=\eta_{0}+\sum_{j=1}^{l} \eta_{j} \hat{u}_{i}^{j}
\label{eq:sdsmucomega}
\end{equation}
...
\begin{equation}
y_{i}^{C}=F^{-1}\left[\Phi\left(Z_{i}\right)\right]
\label{eq:sdsmc}
\end{equation}
...
\begin{equation}
N S E=1-\frac{\sum_{i=1}^{\tau}\left(Y_{i}^{\text {obs }}-Y_{i}^{\text {sim }}\right)^{2}}{\sum_{i=1}^{\tau}\left(Y_{i}^{\text {obs }}-Y^{\text {mean }}\right)^{2}}
\label{eq:nse}
\end{equation}
...
\textbf{Statistical Downscaling Model (SDSM) in \citet{Du2015}}

\citet{Du2015} employed the Statistical Downscaling Model developed by \citet{Wilby2002} as a decision support tool for assessing local climate change impacts.
...
\item
  The use of SDSM allowed \citet{Du2015} to effectively incorporate climate change projections into their nonstationary frequency analysis of low-flow events, providing valuable insights for water resource management under changing climatic conditions.
\item
  It should be noted that there are uncertainties in the downscaled future scenarios of meteorological variables used in this study. These uncertainties could be partially mitigated by exploring additional downscaling methods and different GCMs.
\end{itemize}
...
\caption{Causal process diagram of the impact of drought on the mental health outcomes depression, anxiety and suicide. Numbers in brackets are number of papers meeting the search criteria for each factor. Figure adapted from \citet{vins2015}.}\label{fig:impactDrought}
\end{figure}
...
\caption{\label{tab:tabResults}Effects of hurricane-related stress on PTSD outcome as shown in \citet{kessler2008}.}
\centering
\begin{tabular}[t]{l|l|l}
\hline
Severity & OR & CI\\
...
\caption{Top-level process diagramm depicting linkages between climate change, infratructure and mental health, as shown in \citet{berry2018}.}\label{fig:DAGInfra}
\end{figure}
